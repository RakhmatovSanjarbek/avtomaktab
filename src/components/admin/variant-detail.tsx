...
export function VariantDetail({ variantId }: { variantId: string }) {
  ...
  async function handleUnlink(questionId: string) {
    ...
    try {
      await fetch(`/api/admin/topics/${variantId}/link`, {
        method: "DELETE",
...
function LinkFromTalimDialog({ variantId, onClose, onDone }: { variantId: string; onClose: () => void; onDone: () => void }) {
  const { t, locale } = useLanguage();
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/questions/bank?topicId=${variantId}`);
      const data = await res.json();
  ...
  async function handleSave() {
    ...
    await Promise.all([
      ...toLink.map((questionId) =>
        fetch(`/api/admin/topics/${variantId}/link`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ questionId }) })
      ),
      ...toUnlink.map((questionId) =>
        fetch(`/api/admin/topics/${variantId}/link`, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ questionId }) })
      ),
    ]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 py-8">
      <div className="flex max-h-[85vh] w-full max-w-lg flex-col rounded-2xl border border-border bg-card p-6 shadow-lg">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-base font-semibold text-foreground">Ta'lim moduli bazasidan tanlash</h3>
        </div>
        ...
        <div className="mb-2 flex items-center justify-between text-xs text-muted-foreground">
          ...
          <span className="font-medium text-primary">{selected.size} tanlandi</span>
        </div>
        <div className="flex-1 space-y-1.5 overflow-y-auto">
          {loading ? (
            <p className="py-8 text-center text-sm text-muted-foreground">{t("common.loading")}</p>
          ) : filtered.length === 0 ? (
            <p className="py-8 text-center text-sm text-muted-foreground">Ta'lim moduli bazasida savol yo'q. Avval u yerga savol qo'shing.</p>
          ) : (
            filtered.map((q) => {
              const isChecked = selected.has(q.id);
              return (
                <label key={q.id} className={"flex cursor-pointer items-center gap-3 rounded-xl border p-2.5 transition-colors " + (isChecked ? "border-primary bg-primary/5" : "border-border hover:bg-secondary/50")}>
                  <input type="checkbox" checked={isChecked} onChange={() => toggle(q.id)} className="hidden" />
                  <div className={"flex h-5 w-5 shrink-0 items-center justify-center rounded-md border-2 transition-colors " + (isChecked ? "border-primary bg-primary" : "border-border")}>
