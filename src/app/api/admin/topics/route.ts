import { prisma } from "@/lib/prisma";
import { requireApiSectionAccess } from "@/lib/require-admin";

export async function GET() {
  const session = await requireApiSectionAccess("bosqichli");
  if (!session) return new Response("Forbidden", { status: 403 });

  const topics = await prisma.stage.findMany({
    where: { type: "STAGE" },
    orderBy: { levelOrder: "asc" },
    include: { _count: { select: { questions: true } } },
  });

  return Response.json({
    topics: topics.map((t) => ({ id: t.id, titleJson: t.titleJson, questionsCount: t._count.questions })),
  });
}

export async function POST(req: Request) {
  const session = await requireApiSectionAccess("bosqichli");
  if (!session) return new Response("Forbidden", { status: 403 });

  const body = await req.json();
  const { name } = body;
  if (!name || !name.trim()) return new Response("Missing name", { status: 400 });

  const count = await prisma.stage.count({ where: { type: "STAGE" } });

  const topic = await prisma.stage.create({
    data: { type: "STAGE", levelOrder: count + 1, titleJson: { uzLatin: name, uzCyrl: name, ru: name } },
  });

  return Response.json({ topic });
}
