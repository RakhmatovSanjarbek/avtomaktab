...
const STORAGE_KEY = "mistakes_practice_session";

export default function MistakesPracticeTestPage() {
  const { t } = useLanguage();
  ...
  async function startTest() {
    ...
    setRestored(false);
    try {
      const res = await fetch("/api/mistakes/start", { method: "POST" });
      if (!res.ok) throw new Error();
      const data = await res.json();
      if (!data.questions || data.questions.length === 0) throw new Error("empty");
  ...
  async function finishTest(finalSession: Session) {
    ...
    const res = await fetch("/api/test/submit", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ stageId: null, answers, timeSpentSec }),
    });
    const data = await res.json();
    ...
    return (
      <div className="mx-auto max-w-md py-16 text-center">
        ...
        <div className="mt-8 flex justify-center gap-3">
          <button
            type="button"
            onClick={() => router.replace("/student/profil/xatolar")}
            className="rounded-xl border border-border px-5 py-2.5 text-sm font-medium text-foreground hover:bg-secondary"
          >
  ...
  if (error || !session) {
    return (
      <div className="mx-auto max-w-md py-16 text-center">
        ...
        <p className="text-sm text-muted-foreground">Xatolar topilmadi yoki yuklashda muammo bo'ldi.</p>
        <div className="mt-6 flex justify-center gap-3">
          <Link href="/student/profil/xatolar" className="rounded-xl border border-border px-5 py-2.5 text-sm font-medium text-foreground hover:bg-secondary">
            Ortga
          </Link>
        </div>
      </div>
